...
<fieldset v-if="field.type === 'step'" :aria-labelledby="`${field.name}-legend`">
<legend :id="`${field.name}-legend`">{{ field.label || field.name }}</legend>
<Form
...
:placeholder="field.help || ''"
>
<option key="opt-unselect" selected disabled value="">Choose an option...</option>
...
<div role="radiogroup" :aria-labelledby="field.name + '-legend'">
<legend :id="field.name + '-legend'" class="visually-hidden">{{ field.label }}</legend>
...
:aria-invalid="!!errors[field.name]"
>
<option key="opt-unselected" value="" selected disabled>Choose an option...</option>
<option
...
>
{{ getLabel(field, val) }}
<button type="button" @click="removeMultiSelect(field.name, val)" aria-label="remove">×</button>
</li>
</ul>
</template>
...
const visibleFields = computed(() => props.schema.filter(f => !['submit', 'reset'].includes(f.type)))
const buttonFields = computed(() => props.schema.filter(f => ['submit', 'reset'].includes(f.type)))
...
formData[field.name][child.name] = child.value ?? (child.type === 'checkbox' ? [] : '')
})
} else {
if (field.type === 'checkbox') formData[field.name] = field.value ?? []
else if (field.type === 'file') formData[field.name] = null
else if (field.type === 'multiselectable') formData[field.name] = field.value ?? []
...
let error = ''
if (field.required && (!value || (Array.isArray(value) && !value.length))) error = 'This field is required.'
if ((field.min != null && +value < field.min) || (field.max != null && +value > field.max)) {
if (field.type === 'number' || field.type === 'range') {
if (+value < field.min) error = `Minimum value is ${field.min}.`
if (+value > field.max) error = `Maximum value is ${field.max}.`
}
}
...
if (!hasErrors) emit('submit', JSON.parse(JSON.stringify(formData)))
}
...
if (field.type === 'step') {
field.children.forEach(child => {
formData[field.name][child.name] = child.type === 'checkbox' ? [] : ''
})
} else {
if (field.type === 'checkbox') formData[field.name] = []
else if (field.type === 'file') formData[field.name] = null
else if (field.type === 'multiselectable') formData[field.name] = []
...
formData[name] = event.target.files.length > 1 ? [...event.target.files] : event.target.files[0]
...
return field.options.filter(o => !formData[field.name].includes(o.value))
}
